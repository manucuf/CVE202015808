...
uint8_t Key[CRL_AES192_KEY] = {0x8e, 0x73, 0xb0, 0xf7, 0xda, 0x0e, 0x64, 0x52,0xc8, 0x10, 0xf3, 0x2b, 0x80, 0x90, 0x79, 0xe5,0x62, 0xf8, 0xea, 0xd2, 0x52, 0x2c, 0x6b, 0x7b};
/* Initialization Vector */
uint8_t IV[CRL_AES_BLOCK] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f};
...
int main(void)
{
  ...
  AESctx->mFlags = E_SK_DEFAULT; //AESctx.mFlags = E_SK_DEFAULT;
  /* Set key size to 24 (corresponding to AES-192) */
  AESctx->mKeySize = 24; //AESctx.mKeySize = 24;
  /* Set iv size field to IvLength*/
  AESctx->mIvSize = CRL_AES_BLOCK; // AESctx.mIvSize = CRL_AES_BLOCK;
  /* Initialize the operation, by passing the key.
   * Third parameter is NULL because CBC doesn't use any IV */
  error_status = AES_CBC_Encrypt_Init(AESctx, Key, IV);
  /* check for initialization errors */
  
  if (error_status == AES_SUCCESS) {
    /* Encrypt Data */
    error_status = AES_CBC_Encrypt_Append(AESctx,Plaintext,PLAINTEXT_LENGTH,OutputMessage,&outputLength);
    if (error_status == AES_SUCCESS) {
